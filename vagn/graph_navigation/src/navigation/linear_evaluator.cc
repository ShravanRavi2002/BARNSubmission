...
shared_ptr<PathRolloutBase> LinearEvaluator::FindBest(
  ...
  for (size_t i = 0; i < paths.size(); ++i) {
    ...
    if (paths[i]->Length() <= 0.0f) continue;
    const float path_length = paths[i]->Length() + dist_to_goal[i];
    ConstantCurvatureArc* cca = (ConstantCurvatureArc*) paths[i].get();
    batchmsg[marker] = cca->curvature;
    marker++;
    batchmsg[marker] = path_length;
    marker++;
    batchmsg[marker] = cca->Length();
    marker++;
    batchmsg[marker] = cca->Clearance();
    marker++;
